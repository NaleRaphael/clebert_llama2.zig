...
pub fn initLeaky(allocator: std.mem.Allocator, model_path: []const u8, vocab_size: usize) !Self {
    ...
    const max_word_length = try file.reader().readInt(u32, .little);
    ...
    for (word_scores, 0..) |*word_score, index| {
        word_score.* = @bitCast(try file.reader().readInt(u32, .little));

        const word_length = try file.reader().readInt(u32, .little);
